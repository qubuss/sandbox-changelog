...
public class ChangelogGitService {
    ...
    public List<Issue> connectAndGetAllIssue(URL url){
        List<Issue> result = Lists.newArrayList();
        try {
            result = getAllIssues(getResponseFromGit(url));
        } catch (NieMoznaPobracDanychZGitException nieMoznaPobracDanychZGitException) {
            LOGGER.error(nieMoznaPobracDanychZGitException.getMessage());
        }
        return result;
    }

    private String getResponseFromGit(URL url){
        String response = null;
        try {
            response = changeLogGit.connection(url);
        } catch (NieMoznaSiePolaczycException | IOException nieMoznaSiePolaczycException) {
